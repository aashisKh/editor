...
import {
  ...
  get_roomname,
  get_userid,
} from "./get_name_of_room_and_user.module.js";
...
const focus_next_sibling = (e) => {
  if (e.target.nextSibling) {
    ...
    if (e.target.parentElement.children.length - 1 != index) {
      ...
      handle_focus_element_click(
        previous_active_element,
        currently_active_element
      );
...
const handle_backspace = (e) => {
  if (selection.anchorOffset == 0) {
    ...
    if (index != 0) {
      ...
      socket.emit("Backspace", { room_name: get_roomname(), index });
      ...
      if (text_length > 0) {
        ...
        range.setStart(
          text_container.children[index - 1].childNodes[0],
          text_length
        );
        range.collapse(true);
...
const handle_enter_keydown = (e) => {
  ...
  e.target.innerText = selection.anchorNode.textContent.substring(
    0,
    anchor_offset
  );
...
const handle_document_click = (e) => {
  if (e.target.getAttribute("class") === "editor_div") {
    ...
    lines_container.children[currently_active_element].classList.add(
      "focus_numbers"
    );
    handle_focus_element_click(
      previous_active_element,
...
socket.on("user_typing", (data) => {
  ...
  document.getElementById("cursor_pointer").innerHTML =
